...
export async function register(req: Request, res: Response): Promise<void> {
  ...
  res
    .status(201)
    .json(
      ok(wantsBodyTokens(req) ? { user, accessToken, refreshToken } : { user }),
    );
}
...
export async function refresh(req: Request, res: Response): Promise<void> {
  ...
  res.json(
    ok(
      wantsBodyTokens(req)
        ? { message: "Token refreshed", ...tokens }
...
export async function logout(req: Request, res: Response): Promise<void> {
  const refreshToken = readRefreshToken(req);
  const pushToken = (req.body as { pushToken?: unknown } | undefined)
    ?.pushToken;
  const validPushToken =
    typeof pushToken === "string" &&
    /^Expo(nent)?PushToken\[.+\]$/.test(pushToken)
      ? pushToken
      : undefined;
...
export async function forgotPassword(
  ...
  res.json(
    ok({ message: "If an account exists, a reset link has been sent." }),
  );
}
