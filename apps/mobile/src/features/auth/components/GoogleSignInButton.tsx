import { useState } from 'react';
import { ActivityIndicator, Pressable, Text, View } from 'react-native';
import Svg, { Path } from 'react-native-svg';

import { S } from '@/config/strings';
import { useThemeColors } from '@/features/theme/hooks/useTheme';
import { useAuth } from '../hooks/useAuth';

/** Official multi-color Google "G". */
function GoogleGlyph() {
  return (
    <Svg width={18} height={18} viewBox="0 0 48 48">
      <Path
        fill="#EA4335"
        d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"
      />
      <Path
        fill="#4285F4"
        d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"
      />
      <Path
        fill="#FBBC05"
        d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"
      />
      <Path
        fill="#34A853"
        d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"
      />
    </Svg>
  );
}

/**
 * Google sign-in button. Calls the OAuth flow and reports the result to the
 * caller. When `blocked` (e.g. terms not yet accepted on the register screen),
 * a tap is intercepted and `onBlockedPress` fires instead of the OAuth flow.
 */
export function GoogleSignInButton({
  onSuccess,
  blocked = false,
  onBlockedPress,
}: {
  onSuccess: (isNewUser: boolean) => void;
  blocked?: boolean;
  onBlockedPress?: () => void;
}) {
  const { loginWithGoogle } = useAuth();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const c = useThemeColors();

  const onPress = async () => {
    if (blocked) {
      onBlockedPress?.();
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const res = await loginWithGoogle();
      if (res) onSuccess(res.isNewUser);
    } catch {
      setError(S.googleSignInFailed);
    } finally {
      setLoading(false);
    }
  };

  return (
    <View className="gap-2">
      <Pressable
        onPress={onPress}
        disabled={loading}
        className="flex-row items-center justify-center gap-2.5 border border-border rounded-full h-[52px] bg-card active:opacity-90">
        {loading ? (
          <ActivityIndicator color={c.primary} />
        ) : (
          <>
            <GoogleGlyph />
            <Text className="font-cairo-semibold text-foreground">{S.googleSignIn}</Text>
          </>
        )}
      </Pressable>

      {error ? <Text className="text-xs text-destructive font-cairo text-center">{error}</Text> : null}

      {/* "or" divider — separates the Google button (above) from the form (below) */}
      <View className="flex-row items-center gap-3 my-1">
        <View className="flex-1 h-px bg-border" />
        <Text className="text-xs text-muted-foreground font-cairo">{S.or}</Text>
        <View className="flex-1 h-px bg-border" />
      </View>
    </View>
  );
}
