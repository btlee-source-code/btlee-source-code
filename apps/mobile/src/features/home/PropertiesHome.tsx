...
const rentFetcher = () =>
  propertiesApi.list({ listingType: "rent", limit: 10 }).then((r) => r.data);
const saleFetcher = () =>
  propertiesApi.list({ listingType: "sale", limit: 10 }).then((r) => r.data);
const cityFetcher = () =>
  propertiesApi.list({ governorate: "القاهرة", limit: 10 }).then((r) => r.data);
      ...
      <ResponsivePage size="wide">
        ...
        >
          ...
          <View className="px-6 pt-10 items-center">
            ...
            >
              {S.heroTitleLead}{" "}
              <Text className="text-accent">{S.heroTitleEmphasis}</Text>
            </Text>
          ...
          <View className="mt-8 gap-4">
            <SectionHeader
              title={S.exploreByType}
              onViewAll={() => router.push("/properties")}
            />
              ...
              {CATEGORIES.map((type, i) => (
                <View key={type} style={{ width: categoryWidth }}>
                  <CategoryChip
                    ...
                    index={i}
                    onPress={() =>
                      router.push({ pathname: "/properties", params: { type } })
                    }
                  />
                </View>
              ))}
